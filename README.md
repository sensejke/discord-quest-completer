# Quest Completer v2.2‑fix 🎮

Скрипт для выполнения Discord‑квестов с удобным интерфейсом, авто‑очередью и отдельной вкладкой для ручного забора наград.

## Возможности ✨
- Выполнение видео, игр, стримов и активностей
- Авто‑очередь квестов
- Паузы при 429 и базовая защита от флуда
- Отдельная вкладка «Награды» для ручного забора
- Работа в Desktop и Web (с ограничениями Discord)

## Функции скрипта 🧩
- Автоматическое выполнение квестов: видео, игры, стрим, активности
- Ручной и автоматический забор наград
- Авто‑очередь квестов с переходом к следующему
- Интеллектуальные паузы при 429
- Проверка статуса квеста и прогресса
...
2. Открой DevTools → вкладка Console.  
   ...
   - macOS: Cmd + Option + I
3. Вставь содержимое файла `1.js` в консоль и нажми Enter.

<details>
<summary>Код (открыть/закрыть) 📌</summary>
...
(function () {
    ...
    let cfg = { autoPause: true, pauseAfter: 3, pauseTime: 300, smartSort: true, autoClaim: true };
    ...
    let isClaimedCheck = (qq) => {
        if (!qq) return false;
        return !!(qq?.userStatus?.claimedAt || qq?.userStatus?.rewardClaimedAt);
    };

    let isQuestCompleted = (qq) => {
        if (!qq) return false;
        return !!qq?.userStatus?.completedAt;
    };

    // =========================================================
    // Собираем ВСЕ возможные ID квеста для попыток claim
    // =========================================================
    function collectQuestIds(q) {
        let ids = new Set();
        if (q.id) ids.add(q.id);
        if (q.config?.questId) ids.add(q.config.questId);
        if (q.config?.quest_id) ids.add(q.config.quest_id);
        if (q.config?.id) ids.add(q.config.id);
        // Иногда ID хранится в reward конфиге
        if (q.config?.reward?.questId) ids.add(q.config.reward.questId);
        if (q.config?.rewardConfig?.questId) ids.add(q.config.rewardConfig.questId);
        return [...ids].filter(Boolean);
    }

    // =========================================================
    // Генерируем все возможные эндпоинты для claim
    // =========================================================
    function buildClaimEndpoints(q) {
        let ids = collectQuestIds(q);
        let platform = isApp ? 1 : 0;
        let endpoints = [];

        for (let id of ids) {
            // Новый формат (Discord 2024+)
            endpoints.push({
                label: `claim-reward (${id})`,
            ...
            // Вариант с query param
            endpoints.push({
                label: `claim?location (${id})`,
                url: `/quests/${id}/claim?location=1`,
                body: { platform }
            });
        }

        // Если есть rewardId — пробуем через него
        let rewardId = q.config?.reward?.id || q.config?.rewardConfig?.id || q.config?.rewards?.[0]?.id;
        if (rewardId) {
            for (let id of ids) {
                endpoints.push({
                    label: `rewards/${rewardId}/claim (${id})`,
                    url: `/quests/${id}/rewards/${rewardId}/claim`,
                    body: { platform }
                });
            }
        }
    ...
    const rewardList = $('#rewardList'), rewardsRefreshBtn = $('#rewardsRefresh');
    const captchaNotice = $('#captchaNotice'), captchaTimer = $('#captchaTimer'), captchaMissInfo = $('#captchaMissInfo');
    ...
    $('#cfgAutoClaim').onclick = function () {
        this.classList.toggle('on');
        cfg.autoClaim = this.classList.contains('on');
        if (cfg.autoClaim) { captchaMissCount = 0; log('Автозабор ВКЛ', 'ok'); }
        else log('Автозабор ВЫКЛ', 'info');
        saveCfg();
    };
    ...
    function loadQuests() {
        ...
        if (cfg.smartSort) {
            quests.sort((a, b) => {
                ...
                let tA = cA.tasks[kA]?.target || 999999, tB = cB.tasks[kB]?.target || 999999;
                return tA - tB;
            });
        }
        let runnable = [];
        quests.forEach(q => {
            ...
            let total = qcfg.tasks[key]?.target || 0, done = q.userStatus?.progress?.[key]?.value || 0;
            let pct = Math.floor(done / total * 100), ok = canRun(q), deadline = getDeadline(q);
    ...
    function getRewardLabel(q) {
        ...
        let direct = m?.rewardDescription || m?.rewardDescriptionText || m?.rewardTitle || m?.rewardName || m?.reward || m?.questReward;
        ...
        pushStrings(m); pushStrings(q?.config?.reward); pushStrings(q?.config?.rewardConfig);
        let scoreText = (txt, key = '') => {
            let t = txt.toLowerCase(), s = 0;
            if (/orb|orbs/i.test(t) || /orb/i.test(key)) s += 6;
            if (/nitro|boost/i.test(t)) s += 5;
            if (/reward|награда/i.test(t)) s += 3;
            if (/\d+/.test(t)) s += 2;
            if (/watch|play|stream|video/i.test(t)) s -= 2;
            if (t.length < 4) s -= 2;
            return s;
        };
        let best = strings.map(([k, v]) => ({ k, v, s: scoreText(v, k) })).sort((a, b) => b.s - a.s)[0];
        if (best && best.s > 0 && best.v.trim()) return best.v.trim();
        let reward = q?.config?.reward || q?.config?.rewards?.[0] || q?.config?.rewardConfig;
    ...
    function loadRewards() {
        if (!rewardList || !QuestsStore?.quests) {
            if (rewardList) rewardList.innerHTML = '<div class="qc-empty"><div class="qc-empty-icon">🎁</div><div class="qc-empty-text">Недоступно</div></div>';
            return;
        }
        rewardList.innerHTML = '';
        let rewards = [...QuestsStore.quests.values()].filter(q =>
            q.userStatus?.completedAt && !(q.userStatus?.claimedAt || q.userStatus?.rewardClaimedAt)
        );
        if (rewards.length === 0) {
        ...
        rewards.forEach(q => {
            ...
            btn.onclick = async () => {
                btn.disabled = true;
                btn.textContent = '...';
                // Дебаг в консоль для диагностики
                console.log('[QC DEBUG] Claiming quest:', JSON.parse(JSON.stringify(q)));
                console.log('[QC DEBUG] All IDs:', collectQuestIds(q));
                console.log('[QC DEBUG] Endpoints:', buildClaimEndpoints(q).map(e => e.url));
                try { await claim(q); } finally { btn.disabled = false; btn.textContent = 'Забрать'; }
                loadRewards();
            };
            info.appendChild(name);
            meta.appendChild(done); meta.appendChild(time); meta.appendChild(reward);
    ...
    stopBtn.onclick = () => { stopFlag = true; autoQueue = false; queueBtn.classList.remove('on'); rateLimit.stop(); pauseCard.classList.remove('on'); captchaNotice.classList.remove('on'); log('Стоп', 'warn'); };
    ...
    startBtn.onclick = async () => {
        ...
        captchaNotice.classList.remove('on');
        setStatus('Готов', '');
        let left = loadQuests();
    ...
    // =========================================================
    // CLAIM — пробует все возможные эндпоинты + UI фолбек
    //
    // Порядок:
    // 1. /claim-reward (новый API)
    // 2. /claim (старый API)
    // 3. /claim?location=1
    // 4. /rewards/{rewardId}/claim (если есть rewardId)
    // 5. Всё то же для альтернативных ID
    // 6. UI кнопка "Claim Reward" в интерфейсе Discord
    // 7. FluxDispatcher QUEST_CLAIM_REWARD
    //
    // При 404 — сразу следующий эндпоинт (НЕ retry)
    // При капча — 30 сек ожидания, 2 miss = выключить
    // =========================================================
    async function claim(q) {
        if (!cfg.autoClaim) {
            log('⏭️ Автозабор выключен. Забери вручную (🎁)', 'info');
            return false;
        }
        ...
        // Дебаг
        console.log('[QC] Starting claim for quest:', primaryId);
        console.log('[QC] Quest object:', q);
        console.log('[QC] userStatus:', q.userStatus);

        // Ждём пока сервер подтвердит completedAt
        log('⏳ Ожидаю подтверждение сервера...', 'info');
        setStatus('Ждём сервер', 'синхронизация...', 'rabota');

        let serverOk = false;
        let waitStart = Date.now();
        while (Date.now() - waitStart < 20000 && !stopFlag) {
            let fresh = getQuestFresh(primaryId);
            if (fresh && isQuestCompleted(fresh)) { serverOk = true; break; }
            await sleep(2000);
        }
        if (stopFlag) return false;

        if (serverOk) {
            log('✅ Сервер подтвердил', 'ok');
            await sleep(3000); // Ещё 3 сек для надёжности
        } else {
            log('⚠️ Сервер не подтвердил за 20с. Пробую...', 'warn');
        }

        // Может уже забрана?
        let check = getQuestFresh(primaryId);
        if (isClaimedCheck(check)) {
            log('Награда уже получена! 🎁', 'ok');
            return true;
        }

        // ========================
        // Генерируем все эндпоинты
        // ========================
        let endpoints = buildClaimEndpoints(q);
        log(`📡 Пробую ${endpoints.length} эндпоинтов...`, 'info');
        ...
        for (let ep of endpoints) {
            ...
            // Перед каждой попыткой — проверяем не забрана ли уже
            let freshCheck = getQuestFresh(primaryId);
            if (isClaimedCheck(freshCheck)) {
                log('Награда уже получена! 🎁', 'ok');
                return true;
            }
            ...
            try {
                log(`  → ${ep.label}`, 'info');
                let result = await api.post({ url: ep.url, body: ep.body });
                log('Награда получена! 🎁', 'ok');
                captchaMissCount = 0;
                captchaNotice.classList.remove('on');
                console.log('[QC] Claim SUCCESS via:', ep.url, result);
                return true;
            } catch (e) {
                console.log(`[QC] Claim FAIL ${ep.url}:`, e?.status, e?.body?.message, e?.body);

                // === КАПЧА ===
                if (e?.body?.captcha_sitekey || e?.body?.captcha_rqtoken || e?.body?.captcha_service) {
                    captchaHit = true;
                    all404 = false;
                    let missNum = captchaMissCount + 1;
                    log(`🔐 Капча на ${ep.label}! (${missNum}/2)`, 'warn');

                    if (captchaMissInfo) captchaMissInfo.textContent = `Попытка ${missNum} из 2`;
                    captchaNotice.classList.add('on');
                    setStatus('Капча', 'Пройди капчу за 30 сек', 'pause');

                    let captchaStart = Date.now();
                    let passed = false;
                    while (Date.now() - captchaStart < 30000 && !stopFlag) {
                        let rem = Math.max(0, Math.ceil((30000 - (Date.now() - captchaStart)) / 1000));
                        if (captchaTimer) captchaTimer.textContent = rem;
                        let f = getQuestFresh(primaryId);
                        if (isClaimedCheck(f)) { passed = true; break; }
                        await sleep(1000);
                    }
                    ...
                    if (passed) {
                        captchaMissCount = 0;
                        log('Капча пройдена! 🎁', 'ok');
                        return true;
                    }

                    captchaMissCount++;
                    if (captchaMissCount >= 2) {
                        cfg.autoClaim = false;
                        saveCfg();
                        log('🔕 Автозабор ВЫКЛЮЧЕН (2× капча). Включи в ⚙️', 'warn');
                        let toggle = $('#cfgAutoClaim');
                        if (toggle) toggle.classList.remove('on');
                        return false;
                    }

                    log('Капча не пройдена. Пропускаем.', 'warn');
                    return false;
                }

                // === 404 — пробуем следующий эндпоинт ===
                if (e?.status === 404) {
                    log(`  ✗ 404 на ${ep.label}`, 'warn');
                ...
                // === Другая ошибка ===
                all404 = false;
                log(`  ✗ ${ep.label}: ${e?.body?.message || e?.status}`, 'warn');
                continue;
            }
        }

        // ========================
        // Все API эндпоинты не сработали
        // ========================
        if (all404) {
            log('🔍 Все эндпоинты вернули 404. Пробую через UI...', 'warn');

            // === ПОПЫТКА 1: Найти кнопку в UI ===
            let uiResult = await tryUiClaim(primaryId);
            if (uiResult) return true;

            // === ПОПЫТКА 2: FluxDispatcher ===
            let dispatchResult = await tryDispatchClaim(q);
            if (dispatchResult) return true;

            // === ПОПЫТКА 3: Ждём 15 сек и пробуем снова основные URL ===
            log('⏳ Жду 15 сек — может сервер догонит...', 'info');
            await sleep(15000);
            if (stopFlag) return false;

            let freshAfterWait = getQuestFresh(primaryId);
            if (isClaimedCheck(freshAfterWait)) {
                log('Награда появилась! 🎁', 'ok');
                return true;
            }

            // Финальная попытка — только основные URL
            let finalIds = collectQuestIds(q);
            for (let id of finalIds) {
                for (let path of ['/claim-reward', '/claim']) {
                    try {
                        await api.post({ url: `/quests/${id}${path}`, body: { platform: isApp ? 1 : 0, location: 1 } });
                        log('Награда получена (retry)! 🎁', 'ok');
                        return true;
                    } catch (e) {
                        if (e?.body?.captcha_sitekey) {
                            log('Капча при retry. Забери вручную (🎁)', 'warn');
                            return false;
                        }
                        if (e?.status !== 404) {
                            log(`Retry ${path}: ${e?.body?.message || e?.status}`, 'warn');
                        }
                        continue;
                    }
                }
            ...
            log('❌ Не удалось забрать. Открой карточку квеста в Discord и забери вручную.', 'bad');
            console.log('[QC] CLAIM FAILED — All endpoints returned 404');
            console.log('[QC] Quest dump:', JSON.parse(JSON.stringify(q)));
            console.log('[QC] IDs tried:', collectQuestIds(q));
            return false;
        }

        log('❌ Не забрал. Попробуй вручную (🎁)', 'warn');
        return false;
    }

    // =========================================================
    // Поиск и клик кнопки "Claim Reward" в UI Discord
    // =========================================================
    async function tryUiClaim(questId) {
        log('🖱️ Ищу кнопку в UI Discord...', 'info');

        // Все возможные тексты кнопки (EN, RU, DE, FR, ES, PT, etc)
        let labels = [
            'Claim Reward', 'Claim reward', 'claim reward',
            'Получить награду', 'Забрать награду',
            'Belohnung', 'Réclamer', 'Reclamar', 'Reivindicar',
            'Claim', 'Redeem', 'Accept'
        ];
        ...
        // Ищем по тексту
        btn = [...document.querySelectorAll('button, [role="button"], div[class*="button"]')].find(b => {
        ...
        if (!btn) {
            // Ищем по aria-label
            btn = document.querySelector('[aria-label*="Claim"], [aria-label*="claim"], [aria-label*="Reward"], [aria-label*="reward"]');
        }

        if (!btn) {
            log('Кнопка не найдена. Открой карточку квеста.', 'warn');
            return false;
        }

        log('Нашёл кнопку! Кликаю...', 'info');
        btn.click();

        // Ждём 15 сек — может откроется капча или сразу заберётся
        let start = Date.now();
        while (Date.now() - start < 15000 && !stopFlag) {
            let fresh = getQuestFresh(questId);
            if (isClaimedCheck(fresh)) {
                log('Награда через UI получена! 🎁', 'ok');
                return true;
            }
            await sleep(1500);
        }

        log('UI клик не помог', 'warn');
        return false;
    }

    // =========================================================
    // Попытка через FluxDispatcher (внутренний Discord)
    // =========================================================
    async function tryDispatchClaim(q) {
        if (!FluxDispatcher) return false;

        log('⚡ Пробую через FluxDispatcher...', 'info');

        let actionTypes = [
            'QUEST_CLAIM_REWARD',
            'QUESTS_CLAIM_REWARD',
            'QUEST_REWARD_CLAIM',
            'QUESTS_REWARD_CODE_CLAIM'
        ];
        ...
        for (let type of actionTypes) {
            try {
                ...
                console.log(`[QC] Dispatched ${type}`);
            } catch (e) {
                // Игнорируем — может не быть обработчика
            }
        }

        // Ждём 10 сек
        let start = Date.now();
        while (Date.now() - start < 10000 && !stopFlag) {
            let fresh = getQuestFresh(q.id);
            if (isClaimedCheck(fresh)) {
                log('Dispatch сработал! 🎁', 'ok');
                return true;
            }
            await sleep(2000);
        }

        log('Dispatch не помог', 'warn');
        return false;
    }
    ...
    async function runQuest(q) {
        ...
        if (ok) {
            ...
            log('⏳ Жду 5 сек перед забором...', 'info');
            await sleep(5000);

            let claimed = await claim(q);
            if (!claimed) {
                if (!cfg.autoClaim) log(`Награда за "${name}" ждёт в 🎁`, 'info');
                else log(`Награда за "${name}" не забрана. Попробуй вручную (🎁)`, 'warn');
            }
            loadRewards();
        }
    }
    ...
    async function doPlay(q, total, done) {
        ...
        try {
            ...
            let realGet = RunningGameStore.getRunningGames, realPid = RunningGameStore.getGameForPID;
            ...
            await new Promise(resolve => {
                ...
                const handler = data => {
                    ...
                    updateProg(prog, total); rateLimit.ok();
    ...
    if (!cfg.autoClaim) log('⚠️ Автозабор выключен. Включи в ⚙️', 'warn');
...
## Награды 🎁
Во вкладке «Награды» отображаются завершённые квесты с доступной выдачей.  
Если появляется капча — пройди её вручную, скрипт подождёт и повторит попытку.

## Частые вопросы 🙋‍♂️
**Ctrl + Shift + I не работает**  
Установи Discord PTB или включи DevTools для своей сборки.

**Кнопка «Получить награду» не найдена**  
Открой карточку квеста в интерфейсе Discord и повтори действие.

## Важно ⚠️
Скрипт использует внутренние модули Discord и может ломаться после обновлений.  
Используй на свой риск.

## Благодарности 🤝
- Основа: https://gist.github.com/aamiaa  
- Форк/поддержка: https://github.com/sensejke
