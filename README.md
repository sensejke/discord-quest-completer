...
> Автоматическое выполнение Discord квестов с интерфейсом

![Version](https://img.shields.io/badge/version-2.1-blue)
![Discord](https://img.shields.io/badge/discord-working-green)
![Platform](https://img.shields.io/badge/platform-Desktop%20%7C%20Web-purple)
...
| Функция | Описание |
|---------|----------|
| 🎥 **Видео квесты** | Автоматический просмотр видео |
| 🎮 **Игровые квесты** | Эмуляция запуска игры |
| 📺 **Стрим квесты** | Эмуляция стриминга |
| 🎯 **Активности** | Выполнение activity квестов |
| 🔄 **Авто-очередь** | Последовательное выполнение всех квестов |
| 🛡 **Защита** | Детектор банов и умные паузы |

---

## 🚀 Установка

### Шаг 1: Открой Discord
Перейди на [discord.com/app](https://discord.com/app) или открой десктоп приложение
...
let running = false, stopFlag = false, statsInterval = null
let selected = null, autoQueue = false

const sense = {name: "sensejke", tg: "https://t.me/sensejke", ver: "2"}
...
banWatch.check = function(err) {
	let status = err?.status, text = (err?.body?.message || '').toLowerCase()
...
let isComplete = qid => [...QuestsStore.quests.values()].find(x => x.id === qid)?.userStatus?.completedAt != null
...
const pauseCard = $('#pauseCard'), pauseTime = $('#pauseTime')
...
function updateStats() {
	...
	let sDone = $('#sDone'); if(!sDone) return
...
function loadQuests() {
	...
	if(cfg.smartSort) {
		quests.sort((a, b) => {
			let cA = a.config?.taskConfig ?? a.config?.taskConfigV2, cB = b.config?.taskConfig ?? b.config?.taskConfigV2
	...
	let runnable = []
	quests.forEach(q => {
		let type = getTaskType(q), key = getTaskKey(q)
...
$('#drag').onmousedown = e => { if(e.target.tagName === 'BUTTON') return; drag = true; let r = gui.getBoundingClientRect(); dx = e.clientX - r.left; dy = e.clientY - r.top }
...
startBtn.onclick = async () => {
	...
	showProg(true)
	try { await runQuest(selected) }
	catch(e) { log(`${rnd(msg.pizda)}: ${e.message}`, 'bad'); banWatch.check(e) }
	running = false
...
async function runQuest(q) {
	...
	if(ok) { stats.done++; log(`${rnd(msg.done)} — ${name}`, 'ok'); setStatus('Готово!', '🎉') }
}
...
async function doActivity(q, total) {
	let cid = ChannelStore.getSortedPrivateChannels()[0]?.id ?? Object.values(GuildChannelStore.getAllGuilds()).find(x => x?.VOCAL?.length > 0)?.VOCAL[0]?.channel?.id
...
console.log('%c   by @sensejke', 'font-size:14px;color:#00aced')
console.log('%c  [TG] Telegram: ' + sense.tg, 'font-size:12px;color:#888')
console.log('%c   📜 Original: ' + aamiaa, 'font-size:11px;color:#57F287')
...
### Финал:
![Иллюстрация к проекту](https://i.ibb.co/B5HvzbN2/b443c83a-5a88-49f0-b0e2-4982622da9b2.png)

> [!NOTE]
> Это всего лишь форк [оригинального скрипта](https://gist.github.com/aamiaa/204cd9d42013ded9faf646fae7f89fbb) [ссылка на оригинал автора]

## FAQ

**Q: Ctrl + Shift + I не работает**

A: Либо скачайте [ptb client](https://discord.com/api/downloads/distributions/app/installers/latest?channel=ptb&platform=win&arch=x64), или [используйте](https://www.reddit.com/r/discordapp/comments/sc61n3/comment/hu4fw5x/) для включения DevTools на своей сборке.


**Q: Ctrl + Shift + I делает скриншот**

A: Выключите хоткей в AMD Radeon app.
